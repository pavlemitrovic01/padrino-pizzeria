# STATE-ARCHIVE.md — Padrino batch history

> Rotated out of `workflow/STATE.md` at B19 close (2026-07-27): STATE.md had
> grown to ~36KB against the ~8KB target. STATE.md is injected into context on
> every prompt by the session-bootstrap hook AND read in full by every skill,
> so its size is the workflow's main recurring token cost.
>
> Newest entries go on TOP. The bulk block below preserves its original
> oldest-first chronological order verbatim — moved as a single unit rather
> than reordered, to avoid corrupting ~470 lines of history.
>
> Per-batch audit trail (verify gates, files, SHAs, notes) → `workflow/LOG.md`.
...
(2026-06-18, STRICT, merged 25e5178). Rotated out of the STATE.md
"Poslednji završen" line by the 2-batch cap when B19 closed.

---

## Faza progres — puna hronologija (preneseno doslovno iz STATE.md)
