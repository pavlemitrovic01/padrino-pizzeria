...
**Faza K DONE ✓ (K1 GA4 events, 2026-05-23). Faza L IN PROGRESS — L1+L3+L4 DONE 2026-05-23, L7 (Hero+Navbar) DONE 2026-05-24, L8.0/L8.1/L8.2 (mobile menu redesign sub-program: recon + Topseller/list rows/halal badge + detail sheet) DONE 2026-05-25, L8.3 (mobile menu drawer full-page) DONE 2026-05-25, L8.4 (CartView display-only + edit-reopen + size picker) DONE 2026-05-25. Preostali audit findings (L2/L5/L6) ne queue-uju se kao standardni batch-evi — vidi sekciju "ROADMAP scope" niže (W12 2026-05-23 reframe). Authoritative batch list: STATE.md.**
...
| L4 | Cart item editor mobile compact | STRICT | 4-6h | **DONE 2026-05-23 (SHA e4def5b).** 3 fajla (CartDrawer+CartView+CheckoutView dodato pre-execute kad Pavle screenshot otkrio drugi 3-pill duplikat), +3/-28 net -25 LOC. CartView: lineTotalCents+"Ukupno" item pill removed; h-3→h-32 spacer u canSubmit ternary (code-review CONFIRMED fix). CartDrawer: header py-4→py-2.5 sm:py-3, X red→white/15 grey, "Nazad na meni" button removed (handleGoToMenu RETAINED za success+empty paths). CheckoutView: drugi SUBTOTAL/DOSTAVA/UKUPNO 3-pill inside selectedDeliveryZone removed. Per-batch branch batch/l4-cart-mobile-compact. Code-review 1 CONFIRMED+fixed + 2 PLAUSIBLE deferred. Pavle smoke + Bankart test-mode PASS. |
